...
const townActionPanel = document.querySelector("[data-town-action]");
const townActionMessage = document.querySelector("[data-town-action-message]");
const townEnterButton = document.querySelector("[data-town-enter]");
const townLeaveButton = document.querySelector("[data-town-leave]");
const townLeftName = document.querySelector("[data-town-left-name]");
const townRightName = document.querySelector("[data-town-right-name]");
...
const TOWN_INTERIOR_CELL = "Town Interior";
...
const TOWN_ROAD = [
  { name: "Town Entrance", left: null, right: null },
  { name: "Market Road", left: "Market", right: "Inn" },
  { name: "Guild Road", left: "Armory", right: "Recruitment Office" },
  { name: "Upper Road", left: "Apothecary", right: "Training Ground" },
];
const SCENE_TEMPLATES = {
  ...
  townRoad: {
    title: "Town Road",
    cssClass: "scene-town-road",
    blockedZones: [],
  },
  townInterior: {
    title: "Town Interior",
    cssClass: "scene-town-interior",
    blockedZones: [
      { id: "town-interior-counter", type: "rect", x: 55, y: 45, width: 31, height: 24 },
      { id: "town-interior-shelves", type: "rect", x: 7, y: 12, width: 86, height: 17 },
    ],
  },
  mountains: {
    title: "Mountain Pass",
...
let isInTown = false;
let townRoadIndex = 0;
let selectedTownBuilding = null;
let isInsideTownBuilding = false;
const inventory = [];
...
function getBlockedDirections() {
  if (isInsideHealerHut || isInsideTownBuilding) {
    return [];
  }

  if (isInTown) {
    return ["east", "west"].concat(townRoadIndex === TOWN_ROAD.length - 1 ? ["north"] : []);
  }
...
function renderScene() {
  ...
  resetTrollSprite();
  renderTownLabels();
  sceneTitle.textContent = isInsideTownBuilding
    ? selectedTownBuilding
    : isInTown
      ? TOWN_ROAD[townRoadIndex].name
      : isInsideHealerHut || cell === "Woods" || cell === "Swamp" || cell === "Mountains" || cell === MEADOW_CELL || cell === TROLL_CELL || cell === TEMPLE_CELL || cell === DARK_WIZARD_CASTLE_CELL
    ? template.title
    : cell;
  gridLocation.textContent = isInsideTownBuilding
    ? "Inside town"
    : isInTown
      ? `Town road ${townRoadIndex + 1} of ${TOWN_ROAD.length}`
      : isInsideHealerHut
    ? "Inside the cottage"
...
function renderTownLabels() {
  const townRow = TOWN_ROAD[townRoadIndex];

  townLeftName.textContent = townRow?.left || "";
  townRightName.textContent = townRow?.right || "";
  townLeftName.parentElement.hidden = !townRow?.left;
  townRightName.parentElement.hidden = !townRow?.right;
}

function hideTownAction() {
  townActionPanel.hidden = true;
  townEnterButton.hidden = true;
  townLeaveButton.hidden = true;
}

function showTownEntry(buildingName) {
  selectedTownBuilding = buildingName;
  townActionMessage.textContent = `Enter ${buildingName}?`;
  townEnterButton.hidden = false;
  townLeaveButton.hidden = true;
  townActionPanel.hidden = false;
}
...
function enterTown() {
  ...
  renderScene();
  placePlayer({ x: 50, y: 83 }, false);
  movementStatus.textContent = "You enter town. The road leads north.";
}
...
function leaveTown() {
  ...
  isInsideTownBuilding = false;
  renderScene();
  attemptGridMove("south");
}

function enterTownBuilding() {
  if (!selectedTownBuilding) {
    return;
  }

  isInsideTownBuilding = true;
  renderScene();
  placePlayer({ x: 50, y: 80 }, false);
  townActionMessage.textContent = `${selectedTownBuilding} interior`;
  townEnterButton.hidden = true;
  townLeaveButton.hidden = false;
  townActionPanel.hidden = false;
  movementStatus.textContent = `You enter the ${selectedTownBuilding}.`;
}

function leaveTownBuilding() {
  isInsideTownBuilding = false;
  const buildingName = selectedTownBuilding;
  renderScene();
  placePlayer({ x: buildingName === TOWN_ROAD[townRoadIndex].left ? 32 : 68, y: 67 }, false);
  movementStatus.textContent = `You step back onto ${TOWN_ROAD[townRoadIndex].name}.`;
}

function attemptTownMove(direction) {
  if (isInsideTownBuilding) {
    movementStatus.textContent = "Use the door to return to the road.";
    return;
  }

  if (direction === "east" || direction === "west") {
    movementStatus.textContent = "Buildings block the side streets here.";
    return;
  }

  if (direction === "north") {
    if (townRoadIndex >= TOWN_ROAD.length - 1) {
      movementStatus.textContent = "The town road ends here.";
      return;
    }
    ...
    renderScene();
    placePlayer({ x: 50, y: 83 }, false);
    movementStatus.textContent = `You continue north to ${TOWN_ROAD[townRoadIndex].name}.`;
    return;
  }
  ...
  if (direction === "south") {
    ...
    townRoadIndex -= 1;
    renderScene();
    placePlayer({ x: 50, y: 17 }, false);
    movementStatus.textContent = `You walk south to ${TOWN_ROAD[townRoadIndex].name}.`;
  }
}
...
function attemptGridMove(direction) {
  ...
  if (getCurrentSceneName() === TOWN_CELL) {
    enterTown();
    return;
  }
...
adventureScene.addEventListener("click", (event) => {
  ...
  if (isInTown && !isInsideTownBuilding) {
    const townRow = TOWN_ROAD[townRoadIndex];

    if (townRow.left && isPointInBlockedZone(point, TOWN_LEFT_DOOR_ZONE)) {
      showTownEntry(townRow.left);
      movementStatus.textContent = `${townRow.left} stands to the west.`;
      return;
    }

    if (townRow.right && isPointInBlockedZone(point, TOWN_RIGHT_DOOR_ZONE)) {
      showTownEntry(townRow.right);
